...
class App extends Component {
  ...
  currencyChange (event){
    ...
    if(event.target.value!=='USD' && event.target.value!=='BYN')
    {
      var selectedCurrency = event.target.value;
      var url = `https://api.exchangeratesapi.io/latest?base=USD&symbols=${selectedCurrency}`;
    ...
    {
      var selectedCurrency = event.target.value;
      var url = `http://www.nbrb.by/API/ExRates/Rates/145`;
      var fetchCurrencyRate = this.fetchData(url);
      fetchCurrencyRate
        .then(data => {
          var currency_obj = {
            curr_name: selectedCurrency,
            curr_value: data.Cur_OfficialRate
          };
          console.log('currency', currency_obj);
          this.setState({currency: currency_obj})
        });
